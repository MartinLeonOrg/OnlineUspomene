...
export default function WeddingApp() {
  ...
  useEffect(() => {
    const savedName = localStorage.getItem(`guest-name-${EVENT_SLUG}`);
  ...
  const saveGuestName = () => {
    ...
    localStorage.setItem(`guest-name-${EVENT_SLUG}`, trimmedName);
    ...
    await Promise.all(
      files.map(async (file, i) => {
        try {
          ...
          // 1. Dohvati presigned URL za ovaj event
          const presignResponse = await fetch(`${WORKER}/events/${EVENT_SLUG}/presign`, {
            method: 'POST',
            headers: {
          ...
          // 2. Pošalji samu fotografiju direktno u R2
          ...
          // 3. Nakon uspješnog R2 uploada spremi metadata u D1
          const photoResponse = await fetch(`${WORKER}/events/${EVENT_SLUG}/photos`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          <div style={{ paddingTop: 40 }}>
            ...
            {!done ? (
              ...
              <div style={{ textAlign: 'center', padding: '40px 0' }}>
                ...
                >
                  Vaše fotografije uspješno su dodane u galeriju vjenčanja.
                </p>
                <div
                  style={{
                    display: 'flex',
